  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          "You don't have permission to creates projects in this workspace",
      });
    }
...
export const addMemberProject = async (req, res) => {
  try {
    const { userId } = await req.auth();
    ...
    console.log('Error in  addMemberProject controller', error);
    res.status(500).json({ message: error.code || error.message });
  }
};
